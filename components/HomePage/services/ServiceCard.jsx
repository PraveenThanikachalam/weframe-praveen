...
const ServiceCard = ({ title, img ,url }) => {
  ...
  return (
    <Link ref={ref} className={`w-72 md:w-[30%] transition-all duration-500 z-20 ${inView ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`} href={`${url}`}>
    <div
    ...
    >
      <Image alt='img' src={'/assets/union.svg'} width={500} height={500} className='absolute left-0 right-0 top-0 bottom-0'/>
      {img && (
        <SvgRenderer
          svgText={img}
