        ...
        <div className="right lg:p-5 mt-12 lg:mt-0 lg:w-[50%] w-full h-full flex flex-col  lg:items-start items-center justify-evenly">
          ...
          <div className="my-8 lg:mt-8  w-full flex items-end justify-center">
            <div className="flex lg:flex-row md:flex-row flex-col   w-full items-center lg:justify-end justify-center lg:gap-2 gap-3 ">
              ...
              <Button onClick={sendQuery} label={'Send Query'} variant="filled" />
            </div>
          </div>
        </div>
      </div>

      <div className={`w-[40vw] shadow-sm shadow-cyan-200 absolute bottom-5 z-50 ${show ? '': 'opacity-0'} my-8 h-24 transition-all duration-300 rounded-xl flex items-center justify-center gap-3 bg-gradient-to-b from-[#0e292c] to-[#020C0D] border-2 border-cyan-400`}>
            <div>
              <Image alt='img' src={'/assets/contact/sent.svg'} width={300} height={200} className='h-8 w-auto'/>
            </div>
            <div className='flex flex-col gap-1'>
              <p className='text-cyan-400 font-semibold text-sm'>Message Sent</p>
              <p className='text-sm font-light text-white'>Your message has been sent to the team. They will get back to you soon.</p>
            </div>
      </div>
    </section>
  );
